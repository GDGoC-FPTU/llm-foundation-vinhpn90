...
def compare_models(prompt: str) -> dict:
    ...
    """
    def build_stats(model_name: str, response_text: str, latency: float, usage: dict) -> dict:
    ...
    return {
        "gpt4o": build_stats(OPENAI_MODEL, gpt4o_text, gpt4o_latency, gpt4o_usage),
        "gpt4o_mini": build_stats(OPENAI_MINI_MODEL, mini_text, mini_latency, mini_usage),
        "gemini_flash": build_stats(GEMINI_MODEL, gemini_text, gemini_latency, gemini_usage),
    }
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        ...
        for model_key, model_label in model_labels.items():
            ...
            rows.append(
                "| {prompt} | {model} | {response} | {latency:.2f}s | {input_tokens}/{output_tokens} | ${cost:.8f} |".format(
                    prompt=prompt,
                    model=model_label,
                    response=response,
